...
public weaponbox_touch(weaponbox, id) {
	...
	if(!is_valid_ent(entityID))
		return HAM_IGNORED;
	new max_ammo, weaponType = cs_get_weapon_id(entityID);
	switch(weaponType) {
		case CSW_M249: max_ammo = 100;
		case CSW_P90: max_ammo = 50;
		case CSW_GALI: max_ammo = 35;
		case CSW_MAC10, CSW_AUG, CSW_ELITE, CSW_SG550, CSW_MP5NAVY, CSW_M4A1, CSW_TMP, CSW_G3SG1, CSW_SG552, CSW_AK47: max_ammo = 30;
		case CSW_UMP45, CSW_FAMAS: max_ammo = 25;
		case CSW_FIVESEVEN, CSW_GLOCK18: max_ammo = 20;
		case CSW_P228: max_ammo = 13;
		case CSW_USP: max_ammo = 12;
		case CSW_SCOUT, CSW_AWP: max_ammo = 10;
		case CSW_M3: max_ammo = 8;
		case CSW_DEAGLE, CSW_XM1014: max_ammo = 7;
		default: max_ammo = 1;
	}
	if(weaponType == CSW_C4 || weaponType == CSW_VEST || weaponType == CSW_VESTHELM)
		set_rendering(weaponbox, kRenderFxGlowShell, 0, 255, 0, kRenderNormal, 12);
	else {
		new a = clamp(floatround(255.0/max_ammo*cs_get_weapon_ammo(entityID)), 0, 255);
		set_rendering(weaponbox, kRenderFxGlowShell, colors[a][1], colors[a][0], colors[a][2], kRenderNormal, 12);
	}
	return HAM_IGNORED;
}
